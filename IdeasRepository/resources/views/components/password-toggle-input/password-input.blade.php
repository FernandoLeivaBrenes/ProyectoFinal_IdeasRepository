<div x-data="{ visible: true }" x-init="$watch('visible', ()=> changeVisible_{{ $inputId }}())" class="relative flex flex-col justify-center">
    ...
    <button @click=" visible = ! visible " type="button" class="group absolute w-10 h-10 flex justify-center items-center mt-1 right-px bg-gray-100 rounded-r-md border-l border-solid border-gray-300 focus:outline-none">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 group-hover:stroke-current text-green-500 transition-colors p-auto" fill="none" viewBox="0 0 24 24" stroke="#606060">
